 ...
 */
// Vendored from tesseract-wasm's dist/lib.js, then patched in two ways for
// this app (both applied programmatically, not by hand - see
// generate_ocr_assets.js's sibling patch of tesseract-worker.js for the
// same pattern):
// 1. The trailing `export { ... }` statement was stripped so this can load
//    as a plain classic <script>, same as every other file in this app -
//    OCRClient etc. become ordinary top-level declarations, sharing scope
//    with ocr.js the same way profiles.js's PROFILES is shared with app.js.
// 2. Every `import.meta.url` was replaced with a safe runtime fallback.
//    `import.meta` is a syntax error in a non-module script - it would
//    break parsing of this entire file, not just the dead code path it
//    appears in. Both occurrences are inside code this app never actually
//    calls (a default workerURL, and a wasmBinary-fetch fallback) because
//    ocr.js always supplies workerURL/wasmBinary explicitly - the
//    replacement's exact value doesn't matter, only that it parses.
...
function defaultWorkerURL() {
    return new URL("./tesseract-worker.js", (typeof location !== "undefined" ? location.href : "")).href;
}
...
async function createOCREngine({ wasmBinary, progressChannel, } = {}) {
    if (!wasmBinary) {
        ...
        // `(typeof location !== "undefined" ? location.href : "")` with code that uses `document.currentScript` /
        // `location.href`.
        const wasmURL = resolve(wasmPath, (typeof location !== "undefined" ? location.href : ""));
